...
class pid_controller: public rclcpp::Node
{
    ...
    private:
        ...
        // Private func
        void update_ref(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
        {
            this->reference = *msg;
        }
        ...
        void update_vel()
        {
            ...
            this->current_p = sqrt(pow(this->reference.pose.position.x - this->current.pose.position.x, 2) + pow(this->reference.pose.position.y - this->current.pose.position.y, 2));
            ...
            // Integration
            this->int_p += sign<float>(M_PI/2 - abs(this->current_beta))*(this->current_p - this->previous_p) * this->delta_t;
            ...
            this->cmd_vel.angular.z = this->Kp_beta * current_beta + this->Ki_beta * this->int_beta + this->Kd_beta * this->der_beta;
        }
