...
class occupancyMap {
  ...
  public:
    occupancyMap();
    occupancyMap(float map_width, float map_height, float resolution, int origin_x, int origin_y, float max_range,
                 float min_range, float max_angle, float min_angle, float angle_increment, float initial_probability,
                 float true_positive, float true_negative);
    ...
    void updateMap(geometry_msgs::msg::TransformStamped robot_pose,
